// Code generation is disabled for this module (see codegen.skip.json and
// README.md) — this file is hand-maintained despite the .gen.ts name.
...
export interface UsedForQueryParameter {
  /** Only include views marked as used for the specified purposes. Defaults to [node, edge, all], so record views are excluded unless 'record' is passed. */
  usedFor?: UsedFor[];
}
export interface IncludeInheritedPropertiesQueryParameter {
...
export interface ViewCollectionResponseWithCursorResponse
  extends CursorAndAsyncIterator<ViewDefinition> {}
