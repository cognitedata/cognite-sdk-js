...
describe('Datapoints integration test for data with quality indicators', () => {
  let client: CogniteClient;
  let timeserie: Timeseries;
  ...
  const datapoints = [
    ...
    // Create two data points in November 2022
    {
      timestamp: new Date(2022, 10, 1),
      value: 30,
      status: DataPointStatus.uncertain,
    },
    {
      timestamp: new Date(2022, 10, 2),
      value: 40,
  ...
  test('retrieve aggregates with correct data point status counts', async () => {
    ...
    expect(response[0].datapoints[0].countGood).toBe(0);
    expect(response[0].datapoints[0].countUncertain).toBe(2);
    expect(response[0].datapoints[0].countBad).toBe(0);

    expect(response[0].datapoints[0].countGood).toBe(0);
    expect(response[0].datapoints[0].countUncertain).toBe(0);
    expect(response[0].datapoints[0].countBad).toBe(2);

    expect(response[0].datapoints[0].countGood).toBe(1);
    expect(response[0].datapoints[0].countUncertain).toBe(0);
    expect(response[0].datapoints[0].countBad).toBe(1);

    expect(response[0].datapoints[0].countGood).toBe(1);
    expect(response[0].datapoints[0].countUncertain).toBe(1);
    expect(response[0].datapoints[0].countBad).toBe(0);
  });
});
