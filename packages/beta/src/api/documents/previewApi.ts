...
export class PreviewAPI extends BaseResourceAPI<Any> {
  public document = (id: DocumentId, accepts: acceptsType = "image/png", page: number = 0): Promise<Any> => {
    return this.documentEndpoint<Any>(id, accepts, page);
  }

  public temporaryLink = (documentId: DocumentId): Promise<DocumentPreviewTemporaryLink> => {
    return this.temporaryLinkEndpoint<DocumentPreviewTemporaryLink>(documentId);
  }

  private async documentEndpoint<ResponseType>(id: DocumentId, accepts: acceptsType = "image/png", page: number = 0): Promise<ResponseType> {
  ...
  private async temporaryLinkEndpoint<ResponseType>(documentId: DocumentId): Promise<ResponseType> {
