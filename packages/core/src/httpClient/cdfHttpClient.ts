...
export class CDFHttpClient extends RetryableHttpClient {
  static #serializeQueryParameters(
  ...
  static #isSameOrigin(baseUrl: string, url: string) {
  ...
  static #filterHeaders(headers: HttpHeaders, names: string[]) {
    return names.reduce(
  ...
  static #filterHeader(headers: HttpHeaders, name: string) {
  ...
  #enrichWithOneTimeHeaders(headers?: HttpHeaders) {
  ...
  #response401Handler: Response401Handler = (_, __, ___, reject) => reject();

  #preventTokenLeakage(headers: HttpHeaders, path: string) {
