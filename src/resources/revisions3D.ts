...
import {
  generateListEndpoint, generateCreateEndpoint, generateUpdateEndpoint, generateDeleteEndpoint, generateRetrieveSingleEndpoint
} from '../standardMethods';
import { projectUrl } from '../utils';
import { Revision3DListRequest, Revision3D , CreateRevision3D, CogniteInternalId, UpdateRevision3D, InternalId, List3DNodesQuery, Node3D } from '../types/types';
...
export interface Revisions3DAPI {
   ...
   * ```
   */
  list: (modelId:CogniteInternalId, filter?: Revision3DListRequest) => CogniteAsyncIterator<Revision3D>;
  /**
   ...
   * ```
   */
  create: (modelId:CogniteInternalId, items: CreateRevision3D[]) => Promise<Revision3D[]>;
  /**
   ...
   * ```
   */
  update: (modelId:CogniteInternalId, items: UpdateRevision3D[]) => Promise<Revision3D[]>;
  /**
   ...
   * ```
   */
  retrieve: (modelId:CogniteInternalId, revisionId: CogniteInternalId) => Promise<Revision3D>;
  /**
...
export function generateRevisions3DObject(
  ...
  function parameterizePath(modelId: CogniteInternalId, revisionId?: CogniteInternalId, nodeId?: CogniteInternalId) {
  ...
  return {
    ...
    delete: (modelId:CogniteInternalId, ids: InternalId[]) => {
      return generateDeleteEndpoint(instance, parameterizePath(modelId), map)(ids)
    },
    retrieve: (modelId:CogniteInternalId, revisionId: CogniteInternalId) => {
      return generateRetrieveSingleEndpoint<CogniteInternalId, Revision3D>
        (instance, parameterizePath(modelId), map)(revisionId)
    },
    list3DNodes: (modelId:CogniteInternalId, revisionId: CogniteInternalId, query: List3DNodesQuery = {}) => {
      return generateListEndpoint<List3DNodesQuery, Node3D>
        (instance, parameterizePath(modelId, revisionId), map, false)(query)
    },
    list3DNodeAncestors: (modelId:CogniteInternalId, revisionId: CogniteInternalId, nodeId: CogniteInternalId, query: List3DNodesQuery = {}) => {
      return generateListEndpoint<List3DNodesQuery, Node3D>
        (instance, parameterizePath(modelId, revisionId, nodeId), map, false)(query)
    },
  };
}
